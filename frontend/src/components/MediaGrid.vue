...
<template>
  <div>
    <div v-if="items.length === 0" class="text-sm opacity-60 p-4">
      <span v-if="store.previewLoading">Loading previews…</span>
      <span v-else-if="!store.folder && store.pending.length === 0">Choose a download folder to see existing media.</span>
      <span v-else>No media files yet.</span>
    </div>
    <div class="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 gap-2">
      <MediaThumb v-for="it in visible" :key="it.key" :item="it" />
    </div>
    <div v-if="page < pageCount - 1" class="flex justify-center p-4">
      <button class="btn btn-sm" @click="page++">Load more ({{ items.length - visible.length }} remaining)</button>
    </div>
  </div>
</template>
